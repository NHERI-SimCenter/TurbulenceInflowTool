...
void Foam::digitalFilterFvPatchVectorField::initialiseDigitalFilter()
{
    ...
    forAll (Lu, label)
    {
        ny_[label] = vector(round(Lu[label].component(1)/delta_),round(Lv[label].component(1)/delta_),round(Lw[label].component(1)/delta_))+vector::one;
        nz_[label] = vector(round(Lu[label].component(2)/delta_),round(Lv[label].component(2)/delta_),round(Lw[label].component(2)/delta_))+vector::one;
    }
...
:
    ...
    calculateI_(false),
    calculateL_(false),
    U_(calculateOrRead<vector>("U", inflowProperties_, calculateU_)),
    I_(calculateOrRead<symmTensor>("I", inflowProperties_, calculateI_)),
    Lu_(calculateOrRead<vector>("Lu", inflowProperties_, calculateL_)),
    Lv_(calculateOrRead<vector>("Lv", inflowProperties_, calculateL_)),
    Lw_(calculateOrRead<vector>("Lw", inflowProperties_, calculateL_)),
    Lund_(p.size(), pTraits<tensor>::zero),
