...
Foam::vector Foam::syntheticEddyFvPatchVectorField::uDashEddy
(
...
Foam::scalar
Foam::syntheticEddyFvPatchVectorField::sigmaToLengthscaleRatio() const
{
...
:
    ...
    calculateI_(false),
    calculateL_(false),
    U_(calculateOrRead<vector>("U", dict, calculateU_)),
    I_(calculateOrRead<symmTensor>("I", dict, calculateI_)),
    R_(mag(U_)*I_),
    Lu_(calculateOrRead<vector>("Lu", dict, calculateL_)),
    Lv_(calculateOrRead<vector>("Lv", dict, calculateL_)),
    Lw_(calculateOrRead<vector>("Lw", dict, calculateL_)),
