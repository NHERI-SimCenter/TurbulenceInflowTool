...
void Foam::turbulentDFSEMInletFvPatchVectorField::initialisePatch()
{
    ...
    forAll(patch, faceI)
    {
        ...
        forAll(f, pi)
        {
            polygonPoints[pi] = points[f[pi]];
        }  

        const List<triFace> triPoints = polygonTriangulate().triangulate(polygonPoints, true, true);

        forAll(triPoints, trii)
        {
            tris.append(triPoints[trii]);
        }
...
) const
{
    ...
    distributionMap map(segmentI, std::move(sendMap), std::move(constructMap));
